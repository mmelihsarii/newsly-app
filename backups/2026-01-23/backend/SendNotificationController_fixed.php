...
class SendNotificationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // KATEGORİ - type category ise zorunlu, değilse 0
        $category_id = $type == 'category' ? ($request->category_id ?? 0) : 0;
        
        // ALT KATEGORİ - OPSİYONEL - boş veya null ise 0 yap
        $subcategory_id = $request->subcategory_id;
        if (empty($subcategory_id) || $subcategory_id === '' || $subcategory_id === null) {
            $subcategory_id = 0;
        }
        
        // HABER - OPSİYONEL - boş veya null ise 0 yap
        $news_id = $request->news_id;
        if (empty($news_id) || $news_id === '' || $news_id === null) {
            $news_id = 0;
        }
