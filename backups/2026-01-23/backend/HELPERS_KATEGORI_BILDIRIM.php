<?php
/**
 * KATEGORİYE GÖRE BİLDİRİM GÖNDERME
 * 
 * Bu fonksiyonu helpers.php dosyasındaki send_notification fonksiyonuyla DEĞİŞTİR
 * Dosya yolu: /home/newslyco/public_html/admin/app/helpers.php
 * 
 * Değişiklik: Eğer category_id varsa, o kategorinin topic'ine gönderir
 * Topic formatı: category_{id} (örn: category_3 = Gündem)
 */
...
if (!function_exists('send_notification')) {
    function send_notification($fcmMsg, $language_id = null, $location_id = null, $tokens = null)
    {
        try {
            ...
            // Eğer kategori seçilmişse, o kategorinin topic'ine gönder
            if ($type == 'category' && $category_id > 0) {
                $topic = 'category_' . $category_id;
                \Log::info("Bildirim kategori topic'ine gönderiliyor: $topic");
            } else {
                // Genel bildirim - tüm kullanıcılara
                $topic = 'Turkish';
                \Log::info("Bildirim genel topic'e gönderiliyor: $topic");
            }
            ...
            if (!empty($tokens) && is_array($tokens)) {
                // Token'lara gönder (registration_ids)
                unset($message['to']);
