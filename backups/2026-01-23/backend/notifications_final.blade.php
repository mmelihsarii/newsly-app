                                ...
                                <div class="row" id="type_category" style="display: none">
                                    ...
                                    <div class="form-group col-md-6 col-sm-12">
                                        ...
                                        <select id="news_id" name="news_id" class="form-control">
                                            <option value="">Once kategori secin</option>
                                        </select>
                                    </div>
                                </div>
...
$(document).on('change', '#language_id', function() {
    ...
    if (language_id) {
        fetchList('{{ route("get_category_by_language") }}', {language_id: language_id}, '#category_id');
    }
    $('#news_id').empty().append('<option value="">Once kategori secin</option>');
});
...
$(document).on('change', '#category_id', function() {
    ...
    if (category_id) {
        $.ajax({
            ...
            dataType: 'json',
            beforeSend: function() {
                $('#news_id').empty().append('<option value="">Yukleniyor...</option>');
            },
            success: function(response) {
                $('#news_id').empty().append('<option value="">Haber Secin</option>');
                if (response && response.data) {
                    $.each(response.data, function(i, item) {
                        $('#news_id').append('<option value="'+item.id+'" data-title="'+item.title+'">'+item.title+'</option>');
                    });
                }
            },
            error: function(xhr, status, error) {
                console.log('Hata:', error);
                $('#news_id').empty().append('<option value="">Hata olustu</option>');
            }
        });
    }
});
...
$(document).on('change', '#news_id', function() {
    ...
    if (title && $('#notification_title').val() === '') {
        $('#notification_title').val(title);
    }
    if (title && $('#notification_message').val() === '') {
        $('#notification_message').val('Detaylar icin tiklayin');
    }
});
