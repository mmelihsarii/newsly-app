...
$(document).on('change', '#category_id', function() {
    ...
    // Önce GET dene, hata verirse POST dene
    $.ajax({
        url: '{{ route("get_news_by_category") }}',
        type: 'GET',
        data: { category_id: category_id },
        dataType: 'json',
        success: function(response) {
            console.log('GET başarılı:', response);
            fillNewsSelect(response);
        },
        error: function(xhr, status, error) {
            console.log('GET hata, POST deneniyor...', error);
            // GET başarısız, POST dene
            $.ajax({
                url: '{{ route("get_news_by_category") }}',
                type: 'POST',
                data: { 
                ...
                error: function(xhr2, status2, error2) {
                    console.log('POST de hata:', error2);
                    $('#news_id').empty().append('<option value="">Hata: Haberler yüklenemedi</option>');
                }
            });
        }
    });
...
// Haber select'ini doldur
function fillNewsSelect(response) {
    $('#news_id').empty().append('<option value="">Haber Seçin</option>');
    ...
    if (Array.isArray(data) && data.length > 0) {
        $.each(data, function(i, item) {
            var id = item.id;
            var title = item.value || item.title || item.name || 'Haber #' + id;
            $('#news_id').append('<option value="' + id + '">' + title + '</option>');
        });
    } else {
        $('#news_id').empty().append('<option value="">Bu kategoride haber yok</option>');
    }
}
...
$(document).on('change', '#news_id', function() {
    ...
    if (title && title !== 'Haber Seçin' && title !== 'Önce kategori seçin' && title !== 'Yükleniyor...' && title !== 'Bu kategoride haber yok') {
...
function queryParams(p) {
    return { sort: p.sort, order: p.order, limit: p.limit, offset: p.offset, search: p.search };
}
